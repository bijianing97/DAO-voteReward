...
export function handleStake(event: Stake): void {
  ...
  if (voter) {
    ...
    if (voterInfo) {
      ...
      voterInfo.commissionAddress = stakemanager.validators(event.params.validator).value1
...
export function handleStartUnstake(event: StartUnstake): void {
  ...
  const shareContract = ERC20.bind(Address.fromString(sharesAddress.toString()))
